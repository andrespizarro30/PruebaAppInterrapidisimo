package com.afsoftwaresolutions.pruebaappinterrapidisimo.ui.home.tables
...
import com.afsoftwaresolutions.pruebaappinterrapidisimo.databinding.FragmentTablesBinding
...
import com.afsoftwaresolutions.pruebaappinterrapidisimo.ui.home.tables.adapter.TablesRVAdapter
...
class TablesFragment : Fragment() {
    ...
    private fun initViewModelListener(){
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED){
                homeViewModel.stateSchemasData.collect{state->
                    when(state){
                        HomeStates.Loading -> loadingState()
                        is HomeStates.ErrorSchemasData -> errorState(state)
                        is HomeStates.SuccessSchemasData -> successState(state)
                        else ->{}
                    }
                }
            }
    ...
    private fun errorState(state: HomeStates.ErrorSchemasData){
        binding.pb.isVisible = false
        DialogHelper.showDialog(requireContext(),getString(R.string.fetch_error),state.error)
    }


    private fun successState(state: HomeStates.SuccessSchemasData){
        binding.pb.isVisible = false
        tablesRVAdapter.updateList(state.schemas)
    }
